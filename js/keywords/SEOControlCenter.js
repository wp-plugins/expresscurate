var SEOControl = (function (jQuery) {
            ...
            jQuery.each(jQuery(keywords), function (index, value) {
                if (value.length > 2) {
                    var myRegExp = new RegExp('((^|\\s|>|))(' + value + ')(?=[^>]*(<|$))(?=(&nbsp;|\\s|,|\\.|:|!|\\?|\'|\"|\\;|.?<|$))', 'gmi'),
                        ...
                        title = (numOccurencesTitle > 0 ? "yes" : "no"),
                    ...
                    if (inContent == 0) {
                        inContent = Math.round(parseInt(inContent));
                    }
    ...
    function setupSEOControl() {
        setupKeywords();
        var $metaDescription = jQuery('textarea[name=expresscurate_description]'),
            $addKeywordsWrap = jQuery('.addKeywords'),
            $widgetWraper = jQuery('.expresscurate_widget_wrapper'),
            $addInput = jQuery('.addKeywords input'),
            $keywordWrap = jQuery('.statisticsTitle span'),
            $description = $widgetWraper.find('.description'),
            $descriptionContainer = $description.find('.descriptionWrap');
        if (jQuery.trim($metaDescription.val()) === '') {
        ...
        jQuery('.expresscurate_widget_wrapper .addKeywords input').on("keyup", function (e) {
            ...
            var $list = $addKeywordsWrap.find('.suggestion');
            if (e.keyCode === 13) {
                $list.remove();
                insertKeywordInWidget(KeywordUtils.multipleKeywords($addInput, $keywordWrap), $addKeywordsWrap);
                $addKeywordsWrap.find('.suggestion').hide();
            } else {
        ...
        jQuery('.expresscurate_widget').on("click", '.suggestion li', function () {
        ...
        jQuery('#title').on("keyup", function (e) {
            if (e.altKey && e.keyCode === 75) {
                ...
                if (jQuery('#expresscurate_widget').length && keyword.length > 3) {
                    ...
                    SEOControl.insertKeywordInWidget(KeywordUtils.multipleKeywords($addInput, $keywordWrap), $addKeywordsWrap);
                }
            }
        });
        ...
        $widgetWraper.on('click touchend', '.expresscurate_background_wrap .close', function () {
            KeywordUtils.close(jQuery(this).parent().find('.statisticsTitle').text(), jQuery(this).parents('.expresscurate_background_wrap'));
        });
        /*meta description*/
        jQuery('.description textarea').on('keyup focus', function () {
            if ($widgetWraper.length) {
                var maxVal = 156,
                    $descriptionTextarea = jQuery('.description  textarea'),
                    ...
                    defKeywords = jQuery('#expresscurate_defined_tags').val();
        ...
        // keyboard navigation
        KeywordUtils.suggestionsKeyboardNav(jQuery('#expresscurate_widget').find(' .addKeywords input'));
        //
        ...
        /*Advanced SEO*/
        jQuery('#expresscurate_advanced_seo_nofollow, #expresscurate_advanced_seo_noindex').on('change', function () {
